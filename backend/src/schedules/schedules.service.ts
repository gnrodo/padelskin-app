...
export class SchedulesService {
  ...
  async create(createScheduleDto: CreateScheduleDto): Promise<Schedule> {
    ...
    const existingSchedule = await this.scheduleModel.findOne({ club: createScheduleDto.club }).exec();
    if (existingSchedule) {
      throw new BadRequestException(`Schedule already exists for club ID "${createScheduleDto.club}". Use update instead.`);
    }
  ...
  async findOne(id: string): Promise<Schedule> {
    const schedule = await this.scheduleModel.findById(id).populate('club', 'name slug').exec();
    if (!schedule) {
  ...
  async findOneByClub(clubId: string): Promise<Schedule> {
    const schedule = await this.scheduleModel.findOne({ club: clubId }).populate('club', 'name slug').exec();
     if (!schedule) {
  ...
  async update(id: string, updateScheduleDto: UpdateScheduleDto): Promise<Schedule> {
    ...
    if (updateScheduleDto.club) {
        throw new BadRequestException('Changing the club of a schedule is not allowed via update.');
    }
